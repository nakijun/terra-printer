...
TeExternalTheme& TeExternalTheme::operator=(TeExternalTheme& rhs)
{
	if ( this != &rhs )
	{
		*(TeTheme*)this = rhs;
		...
		this->remoteTheme_ = rhs.remoteTheme_;
		this->sourceDB_ = rhs.sourceDB_;
	}
...
TeViewNode* TeExternalTheme::clone()
{
	...
	*extTheme = *this;
	...
	extTheme->remoteTheme_ = (TeTheme*)this->remoteTheme_->clone();
...
{
	return save(sourceDB_, useCollection_);
}
...
bool TeExternalTheme::beforeCopyThemeTo(TeAbstractTheme* absThemeCopy, TeDatabase* outputDatabase)
{
	if(TeTheme::beforeCopyThemeTo(absThemeCopy, outputDatabase) == false)
	{
		return false;
	}

//Teste se o tema eh realmente um TeExternalTheme
	TeExternalTheme* themeCopy = dynamic_cast<TeExternalTheme*>(absThemeCopy);
	if(themeCopy == 0)
	{
		return false;
	}
	...
	if(themeCopy->createExternalThemeTable(outputDatabase) == 0)
	{
		return false;
	}

	TeDatabase* remoteDb = themeCopy->getRemoteTheme()->layer()->database();
	...
	themeCopy->setSourceDatabase(outputDatabase);
	themeCopy->setRemoteDbId(dbId);
